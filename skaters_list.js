...
//    $(td[1]).remove()
    ...
    $(td[5]).css({width:'16px', backgroundColor: $(td[5]).text() == "○" ? 'orange' : 'inherit'}).text($(td[5]).text().replace(/○|備考/,"備"))
    $(td[6]).text($(td[6]).text().replace(/^\s*$/,"仮").replace(/◎/,"-").replace(/登録/,"登")).css({width:'16px', backgroundColor: $(td[6]).text() == "仮" ? 'orange' : 'inherit'})
